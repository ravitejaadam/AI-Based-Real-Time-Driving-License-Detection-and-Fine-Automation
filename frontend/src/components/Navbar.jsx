...
import { Search, Bell, User, Circle, LogOut } from 'lucide-react';
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3 text-sm text-white">
          <div className="text-right hidden sm:block">
            ...
            <p className="text-[10px] text-gray-500">{user?.role === 'super_admin' ? 'Super Admin' : 'Admin'}</p>
          </div>
          <button
            onClick={logout}
            className="flex items-center gap-2 rounded-full border border-white/10 bg-surface px-3 py-2 text-gray-200 hover:border-primary hover:text-white transition"
          >
            <LogOut size={16} />
